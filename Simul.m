function [obj,output] = Simul(plan,para,Nsim)
Nday = size(plan,2);
adjPlan = repmat(plan,1,1,Nsim);
Inventory = zeros(para.Nprod,size(para.order,2)+1,Nsim); InventoryCost = zeros(Nsim,size(para.order,2)+1);
...
parfor sim = 1:Nsim    
    ...
    cTime = nan(1,Nday); fTime = nan(1,Nday);
    lastFail = para.Fail.lastFail; lastFail(isnan(lastFail)) = 0;    
    seq = cell(1,Nday); sTime = cell(1,Nday); eTime = cell(1,Nday);
    for dd = 1:Nday        
    ...
    % Assume that 1 day consists only 14 hrs (840 mins) working. Then all jobs
        ...
        startT = startT(1:end-length(remProd),:); endCutT = endT(1:end-length(remProd),:);
        ...
        seq{dd} = task; sTime{dd} = startT; eTime{dd} = endT;
        if ~isempty(task)            
            cTime(dd) = max(endT(:)); fTime(dd) = max(endCutT(:));   
        end
    end  
    ...
    if mod(sim,100) == 0
        display(['Run up to ',num2str(sim),' simulations'])
    end
    adjPlan(:,:,sim) = plan_sim;
    Inventory(:,:,sim) = Inv; InventoryCost(sim,:) = InvCost;
    FailRecord(:,:,sim) = lastFail;
    sequence{sim,:} = seq; startTime{sim,:} = sTime; endTime{sim,:} = eTime;
    completeTime(sim,:) = cTime ; finTime(sim,:) = fTime;
end
obj = mean(sum(InventoryCost,2));
output = struct('Inventory',{Inventory},...
